...
<body>
    <div class="container">
        <form action="/gestion-ecole/public/index.php?action=update&role=<?= $role ?>&id=<?= $users["id"]?>" method="POST" onsubmit="return validateForm()">
                    ...
                    <div class="col-sm-4 form-group">
                        ...
                        <option value="" disabled selected>Sélectionner une classe</option>
                            <?php foreach ($allClass as $classe) { ?>
                                        <option value="<?= htmlspecialchars($classe['id']) ?>" <?= $classe['id'] == $users['id_classe'] ? 'selected' : '' ?>><?php echo $classe["nom_classe"]?></option>   
                                    <?php } ?>
                        </select>
